...
def draw_keypoints_in_image_pair(img, pix_pos, img_number):
    ...
    for i in range(len(pix_pos)):
        img = cv2.circle(img, (int(pix_pos[i, 0]), int(pix_pos[i, 1])), radius, color_BGR, thickness)
    plt.imshow(img)
    plt.show()
    plt.imsave(f'{time.time()}_{img_number}.png', img)
    return color_BGR, cv2, img, radius, thickness, time


def draw_keypoints_in_evaluation_image(imname, kpts):
    ...
    for i in range(len(kpts)):
        cv2.circle(image, (int(kpts[i][0]), int(kpts[i][1])), radius, color_BGR, thickness)
    plt.imshow(image)
    plt.show()
    plt.imsave(f'{time.time()}.png', image)