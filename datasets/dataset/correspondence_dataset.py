...
class CorrespondenceDataset(Dataset):
    ...
    def decode(self, data):
        if data['type']=='homography':
            ...
            img0, img1, pix_pos0, pix_pos1, H = self.decode_homography(data)            
        elif data['type']=='gl3d':
            # img0, img1, pix_pos0, pix_pos1, H, scale_offset, rotate_offset = self.decode_gl3d(data)
            img0, img1, pix_pos0, pix_pos1, H = self.decode_gl3d(data)            
        else:
        ...
        rotation = torch.tensor( -np.degrees(np.arctan2(-H[0,1], H[0,0])), dtype=torch.float32)
        H=torch.tensor(H,dtype=torch.float32)

        # ret = {'img_list0':img_list0,'pts_list0':pts_list0,'pix_pos0':pix_pos0,'grid_list0':grid_list0,
        #     'img_list1':img_list1,'pts_list1':pts_list1,'pix_pos1':pix_pos1,'grid_list1':grid_list1,
        #     'scale_offset':scale_offset,'rotate_offset':rotate_offset,'H':H}

        # ret = {'img0': img0, 'pix_pos0':pix_pos0,
        #     'img1':img1, 'pix_pos1':pix_pos1, 'H':H}

        ret = {'image1': img0, 'pts1':pix_pos0, 'rotation': rotation,
            'image2':img1, 'pts2':pix_pos1, 'homography':H}
